...
class Get {
  constructor (key, envs = [], overload = false, ignore = [], DOTENV_KEY = '', all = false, strict = false) {
    this.key = key
    this.envs = envs
  ...
  run () {
    ...
    const { processedEnvs } = new Run(this.envs, this.overload, this.ignore, this.DOTENV_KEY, processEnv).run()
