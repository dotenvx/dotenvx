...
t.test('login and logout remain hidden from default command list', ct => {
  ...
  const localCustody = help.slice(help.indexOf('Local Custody:'), help.indexOf('Managed Custody:'))
  const managedCustody = help.slice(help.indexOf('Managed Custody:'))
  ct.notMatch(help.slice(help.indexOf('Local Custody:')), /\n\s+login\s+/, 'root help does not advertise login')
  ct.notMatch(help.slice(help.indexOf('Local Custody:')), /\n\s+logout\s+/, 'root help does not advertise logout')
  ct.match(localCustody, /\n\s+lock\s+⊡ lock private keys with a local passphrase/, 'local custody advertises lock')
  ct.match(localCustody, /\n\s+native\s+⌥ move private keys in\/out of your OS secret store/, 'local custody advertises native')
  ct.notMatch(localCustody, /\n\s+armor\s+/, 'local custody does not advertise armor')
  ct.notMatch(localCustody, /\n\s+curl\s+/, 'local custody does not advertise curl')
  ct.match(managedCustody, /\n\s+armor\s+⛨ move private keys in\/out of Dotenvx Armor/, 'managed custody advertises armor')
  ct.match(managedCustody, /\n\s+curl\s+⛨ call authenticated api Dotenvx Armor/, 'managed custody advertises curl')
  ct.end()
})

t.test('default help lists direct utility commands with ls first after keypair', ct => {
  const help = childProcess.execFileSync(process.execPath, [path.join(__dirname, '../../src/cli/dotenvx.js'), '--help'], { encoding: 'utf8' })
  const commands = help.slice(help.indexOf('Commands:'), help.indexOf('Local Custody:'))
