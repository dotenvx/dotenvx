...
t.test('login and logout remain hidden from default command list', ct => {
  ...
  const hiddenMenu = childProcess.execFileSync(process.execPath, [path.join(__dirname, '../../src/cli/dotenvx.js'), 'hidden'], { encoding: 'utf8' })
  ct.match(hiddenMenu, 'Hidden Commands:')
  ct.match(hiddenMenu, /Hidden Commands:\n\s+protect\s+/, 'protect is the first hidden command')
  ct.match(hiddenMenu, /Beta Commands:\n\s+init\s+[^\n]+\n\s+define\s+[^\n]+\n\s+validate\s+[^\n]+\n\nDeprecated Commands:/)
  ct.notMatch(hiddenMenu.split('Beta Commands:')[0], /\n\s+(?:init|define|validate)\s/)
  ct.match(hiddenMenu, /Deprecated Commands:\n\s+precommit \[directory\]\s+deprecated\. use \[dotenvx protect\]/)
  ct.notMatch(hiddenMenu.split('Deprecated Commands:')[0], /\n\s+precommit\s/)
  ct.match(hiddenMenu, /Deprecated Commands:[\s\S]*\n\s+gitignore\s+deprecated\. use \[dotenvx protect\]/)
  ct.notMatch(hiddenMenu.split('Deprecated Commands:')[0], /\n\s+gitignore\s/)
  ct.match(hiddenMenu, /Deprecated Commands:[\s\S]*\n\s+prebuild \[directory\]\s+deprecated\. use \[dotenvx protect --docker\]/)
  ct.notMatch(hiddenMenu.split('Deprecated Commands:')[0], /\n\s+prebuild\s/)
  ct.notMatch(help, /\n\s+precommit\s/)
  ct.notMatch(hiddenMenu, /Local Custody:|Managed Custody:/)
  ct.notMatch(help, /Local Custody:|\n\s+(?:lock|native|1password|bitwarden)\s+/, 'root help does not advertise local custody')
  ct.notMatch(help, /Managed Custody:|\n\s+(?:armor|curl)\s+/, 'root help does not advertise managed custody')
  ct.notMatch(help, /\n\s+login\s+/, 'root help does not advertise login')
  ct.notMatch(help, /\n\s+logout\s+/, 'root help does not advertise logout')
  ct.match(hiddenMenu, /\n\s+lock\s+⊡ lock private keys with a local passphrase/, 'hidden menu advertises lock')
  ct.match(hiddenMenu, /\n\s+native\s+⌥ move private keys in\/out of your OS secret store/, 'hidden menu advertises native')
  ct.match(hiddenMenu, /\n\s+armor\s+⛨ move private keys in\/out of Dotenvx Armor/, 'hidden menu advertises armor')
  ct.match(hiddenMenu, /\n\s+curl\s+⛨ call authenticated api Dotenvx Armor/, 'hidden menu advertises curl')
  ct.end()
})
