...
t.test('buildApiError includes API help in messageWithHelp', ct => {
  ...
  ct.equal(error.message, '[NO_TEAMS] create or join a team before running this command')
  ct.equal(error.help, 'fix: [https://github.com/dotenvx/dotenvx/issues/924]')
  ct.equal(error.messageWithHelp, '[NO_TEAMS] create or join a team before running this command. fix: [https://github.com/dotenvx/dotenvx/issues/924]')
  ct.end()
})
