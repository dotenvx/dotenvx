...
t.test('#decryptKeyValue invalid encrypted value raises error', ct => {
  try {
    ...
    ct.same(error.code, 'MALFORMED_ENCRYPTED_DATA')
    ct.same(error.message, '[MALFORMED_ENCRYPTED_DATA] could not decrypt KEY because encrypted data appears malformed')
  }
...
t.test('#decryptKeyValue invalid privateKey', ct => {
  ...
  try {
    ...
    ct.same(error.code, 'INVALID_PRIVATE_KEY')
    ct.same(error.message, '[INVALID_PRIVATE_KEY] could not decrypt KEY using private key \'DOTENV_PRIVATE_KEY=invalid…\'')
  }
...
t.test('#decryptKeyValue wrong privateKey', ct => {
  ...
  try {
    ...
    ct.same(error.code, 'WRONG_PRIVATE_KEY')
    ct.same(error.message, '[WRONG_PRIVATE_KEY] could not decrypt KEY using private key \'DOTENV_PRIVATE_KEY=9c1ab41…\'')
  }
