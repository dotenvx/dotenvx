...
t.test('#smartPrivateKey', ct => {
  const result = smartPrivateKey(filepath)
...
t.test('#smartPrivateKey when process.env.DOTENV_PRIVATE_KEY is set', ct => {
...
t.test('#smartPrivateKey when process.env.DOTENV_PRIVATE_KEY is set but it is an empty string', ct => {
...
t.test('#smartPrivateKey when .env.keys present', ct => {
...
t.test('#smartPrivateKey when .env.keys present but filename is not .env (use invertse of DOTENV_PUBLIC_KEY* in the filename (if exists))', ct => {
...
t.test('#smartPrivateKey when DOTENV_PRIVATE_KEY passed and custom filename', ct => {
  process.env.DOTENV_PRIVATE_KEY = '<privateKey>'
...
t.test('#smartPrivateKey when DOTENV_PRIVATE_KEY passed and custom filename but custom filename has a different named DOTENV_PUBLIC_KEY', ct => {
  process.env.DOTENV_PRIVATE_KEY = '<privateKey>'
  ...
  const result2 = smartPrivateKey(filepath)
