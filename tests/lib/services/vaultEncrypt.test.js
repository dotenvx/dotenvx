...
t.test('#run', ct => {
  ...
  } = new VaultEncrypt('tests/monorepo/apps/backend', ['.env']).run()
...
t.test('#run (when no .env.vault or .env.keys yet)', ct => {
  const {
    ...
    existingVaults,
    addedDotenvFilenames
  } = new VaultEncrypt('tests/monorepo/apps/frontend', ['.env']).run()
...
t.test('#run (non-existant directory)', ct => {
  try {
    new VaultEncrypt('tests/monorepo/apps/backendzzzz').run()
...
t.test('#run (empty envFile)', ct => {
  try {
    new VaultEncrypt('tests/monorepo/apps/backend', []).run()
    ct.fail('should have raised an error but did not')
  } catch (error) {
...
t.test('#run (envFile not found)', ct => {
  try {
    new VaultEncrypt('tests/monorepo/apps/backend', ['.env.notfound']).run()
    ct.fail('should have raised an error but did not')
  } catch (error) {
...
t.test('#_parsedDotenvKeys (returns empty when no keys file)', ct => {
  const vaultEncrypt = new VaultEncrypt()
...
t.test('#_parsedDotenvKeys (returns parsed keys file when directory passed containing .env.keys file)', ct => {
  ...
  const dotenvKeys = new VaultEncrypt(directory)
...
t.test('#_parsedDotenvVault (returns parsed vaults when directory passed contains .env.vault)', ct => {
  const vaultEncrypt = new VaultEncrypt('tests/monorepo/apps/backend')
  const parsed = vaultEncrypt._parsedDotenvVault()
