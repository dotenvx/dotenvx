...
t.test('#run (finds .env file)',
  async ct => {
    ...
    const originalParsed = parseEnv(fsx.readFileXSync(envFile))
    const originalKeysParsed = parseEnv(fsx.readFileXSync(envKeysFile))
    ...
    const parsedKeys = parseEnv(p1.envKeysSrc)
