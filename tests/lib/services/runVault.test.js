...
t.test('#_inject', ct => {
  ...
  const runVault = new RunVault('tests/monorepo/apps/frontend/.env', null, true)

  const { injected, preExisted } = runVault._inject(processEnv, parsed)
...
t.test('#_inject with pre-existing', ct => {
  ...
  const runVault = new RunVault('tests/monorepo/apps/frontend/.env', null, false)
...
t.test('#_inject with no arguments', ct => {
  ...
  const { injected, preExisted } = runVault._inject()
